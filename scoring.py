...
def _spec():
    """Live market spec, so the context tracks markets.yaml instead of a hardcoded list.

    Falls back to the SEMICON constant below only when the watchtower package is not
    importable (e.g. running a root script standalone outside the project env).
    """
    try:
        from watchtower.config import load_markets
        return load_markets()["semicon"]
...
def _context() -> Counter:
    spec = _spec()
    if spec is not None:
        # single source of truth: watchtower.relevance builds the same vector the
        # SITREPs and the agent council use.
        from watchtower.relevance import context_doc
        return context_doc(spec)
